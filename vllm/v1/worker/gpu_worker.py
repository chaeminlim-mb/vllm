...
class Worker(WorkerBase):
    ...
    def determine_available_memory(self) -> int:
        ...
        # num_gpu_blocks. Required for MoRIIO PD: mismatched per-rank block
        # counts make cross-instance KV reads overflow the smaller memory region
        # at high concurrency ("Session batch read error 11 length out of
        # range"). Mirrors ExecutorWithExternalLauncher.determine_available_memory's
        # TP min, but over the DP group's CPU (gloo) communicator. Disable with
        # VLLM_DP_SYNC_KV_CACHE_NUM_BLOCKS=0.
        if (
            envs.VLLM_DP_SYNC_KV_CACHE_NUM_BLOCKS
