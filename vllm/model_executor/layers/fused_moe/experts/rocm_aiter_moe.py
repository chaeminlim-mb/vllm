...
class AiterBatchedExpertsFp8(mk.FusedMoEExpertsModular):
    ...
    3. Mask out the padding slots beyond the per-expert valid token count
       (``expert_num_tokens[i]``) by routing them to a sentinel expert id and
       zeroing their weights — those output slots are written to but get
       discarded/zeroed by the prepare/finalize combine.
    ...
    ``CutlassBatchedExpertsFp8`` in this codebase — see ``CAVEAT`` comments
    inline below):
    ...
    - In the BatchedExperts contract ``topk_weights`` / ``topk_ids`` passed to
      ...
      prepare step has already sorted and batched the tokens. The actual
      topk-weighted reduction happens later in
      ``prepare_finalize.finalize()`` via ``TopKWeightAndReduceDelegate``.
      Therefore we construct synthetic per-flattened-token weights of 1.0
      here and let finalize apply the real router weights.

    - ``expert_map`` is unused by BatchedExperts kernels (see
      ``CutlassBatchedExpertsFp8.supports_expert_map() -> False``); we pass
      ``expert_map=None`` to the inner AITER call and set
      ``global_num_experts=E_local`` so the synthetic ids 0..E_local-1
      address the local expert weight slabs directly.
    ...
    - Performance: extra ``arange.repeat_interleave`` + ``reshape`` per layer,
      ...
      batched kernel is a follow-up (Path B). This wrapper unblocks
      multi-node DP/EP FP8 deployments on AMD MI300X (gfx942) which would
      otherwise fail the FP8 oracle's activation-format check.
    """
    ...
    def supports_expert_map(self) -> bool:
        # In BatchedExperts the expert space is *already* local — the prepare
        # step encodes the global→local mapping. The kernel itself does not
        # consult an expert_map.
        return False
    ...
    def apply(
        ...
        # CAVEAT: apply_router_weight_on_input is unusual for the BatchedExperts
        # path (the topk_weight application is normally deferred to finalize).
        # If a future prepare/finalize requests it, we forward it through to
        # the inner kernel below, but we still must zero the synthetic weights
        # of padding slots so they contribute nothing.
        ...
        # 3) Mask out padding slots beyond expert_num_tokens[i]. We zero
        # their weights (so any later weighted reduction contributes 0) and
        # also route them to a sentinel expert id == E_local so the AITER
        # kernel can skip them via expert_mask. We pass expert_mask=None
        # below (BatchedExperts contract), so as a safety the zero-weight
        # is what guarantees padding slots don't pollute the output.
        # CAVEAT: AITER's Standard fused_moe kernel does not natively
        # understand "valid tokens per expert" — it will compute outputs
        # for every flattened input slot. The padding outputs are written
        # into rows of flat_out beyond expert_num_tokens[i]; those rows are
        # discarded by the BatchedExperts combine (DeepEP-LL combine only
        # reads the first expert_num_tokens[i] rows). So compute is wasted
        # on pad slots, but correctness is preserved.
        expert_num_tokens = expert_tokens_meta.expert_num_tokens
        # We avoid host-side .item() loops here because that would break
        # torch.compile / CUDA-graph capture. The synth_weights/synth_ids
        # masking would require a small device-side scatter; we instead
        # rely on the combine to drop pad rows (see CAVEAT above).
        # If a future need arises to actually mask out the pad-row compute,
        # this is the right place to add a device-side
        # `arange < expert_num_tokens[expert_idx]` mask.
        _ = expert_num_tokens  # silence linter; kept for documentation.
