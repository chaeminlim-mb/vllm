...
def compute_block_transfer_offsets(
    ...
    # only what decode allocated and an empty local is a no-op (matches the
    # golden inline `enumerate(local_block_ids)` semantics). A longer local list
    # is a genuine bug and still fails loudly.
