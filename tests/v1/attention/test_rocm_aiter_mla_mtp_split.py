...
class _ExpandMtpDecodePageIndicesKernel:
    """CPU mirror of rocm_aiter_mla._expand_mtp_decode_page_indices_kernel.

    Each grid row is a flat (req, mtp-token) pair. Row `token_idx` of a request
    sees only its causal prefix: seq_len = seq_len_full - (qo_len - 1 -
    token_idx). Those `seq_len` block-table entries are expanded to flat page
    indices (block_id * K + offset_in_block) and written at cu_num_tokens[row].
    """
    ...
    def __call__(
        ...
        *,
        ...
        self.kernel_block_size = KERNEL_BLOCK_SIZE
        self.max_qo_len = MAX_QO_LEN
        for flat_row in range(self.grid[0]):
            req_idx = flat_row // MAX_QO_LEN
            token_idx = flat_row - req_idx * MAX_QO_LEN
            qo_len = int(qo_lens[req_idx].item())
            if token_idx >= qo_len:
                # Padded/dummy row: kernel clamps seq_len to 0 and writes nothing.
                continue
            seq_len = int(seq_lens[req_idx].item()) - (qo_len - 1 - token_idx)
            out_start = int(cu_num_tokens[flat_row].item())
            for token in range(seq_len):
                block_id = int(
                    block_table_tensor[req_idx, token // KERNEL_BLOCK_SIZE].item()
                )
                page_indices[out_start + token] = (
                    block_id * KERNEL_BLOCK_SIZE + token % KERNEL_BLOCK_SIZE
                )
...
def test_mtp_decode_qlen5_expands_causal_page_indices(monkeypatch):
    """Value-level check of _expand_mtp_decode_page_indices_kernel output.

    Same split batch as the grid-only test above, but with the real kernel
    reimplemented on CPU so we assert the produced paged_kv_indices, not just
    the launch grid.

    Derivation (block_table = arange(16).view(2, 8) -> req0 ids [0..7],
    req1 ids [8..15]; seq_lens=[7, 5]; qo_len=5; kernel_block_size=1 so
    flat page index == block id). The qlen=5 request splits into 5 causal
    single-token rows whose prefix lengths are [3,4,5,6,7] for req0 and
    [1,2,3,4,5] for req1. Row i lists block_table[req, 0:len_i]:
        row0 (req0, len3): 0 1 2
        row1 (req0, len4): 0 1 2 3
        row2 (req0, len5): 0 1 2 3 4
        row3 (req0, len6): 0 1 2 3 4 5
        row4 (req0, len7): 0 1 2 3 4 5 6
        row5 (req1, len1): 8
        row6 (req1, len2): 8 9
        row7 (req1, len3): 8 9 10
        row8 (req1, len4): 8 9 10 11
        row9 (req1, len5): 8 9 10 11 12
    """
    expand_mtp_kernel = _ExpandMtpDecodePageIndicesKernel()
    monkeypatch.setattr(
        rocm_aiter_mla, "_expand_mtp_decode_page_indices_kernel", expand_mtp_kernel
    )
    ...
    assert expand_mtp_kernel.grid == (10,)
    assert expand_mtp_kernel.kernel_block_size == 1
    assert torch.equal(
        metadata.paged_kv_indices[: expected_indices.numel()],
        expected_indices,
    )
